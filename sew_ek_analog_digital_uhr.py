...
"""
A class that displays a clock rendered in pygame, based on time_manager.TimeManager.
Allows switching between different display modes using the following keys:
A: Switch to analog display mode
D: Switch to digital display mode
P: Switch between continuous and discrete pointer movements
"""
class Clock(object):
    DIGITAL = 0
    ANALOG = 1
    ...
    def run_game(self):
        while self.running:
            ...
            if self.mode == self.ANALOG:
                ...
                pygame.draw.line(self.screen, (50, 255, 0), self.center, (
                (self.center[0] + math.sin(hour_deg) * scale_factor * hour_pointer_length),
                (self.center[1] - math.cos(hour_deg) * scale_factor * hour_pointer_length)))
                pygame.draw.line(self.screen, (255, 0, 0), self.center, (
                (self.center[0] + math.sin(second_deg) * scale_factor * second_pointer_length),
                (self.center[1] - math.cos(second_deg) * scale_factor * second_pointer_length)))
                pygame.draw.line(self.screen, (255, 200, 0), self.center, (
                (self.center[0] + math.sin(minute_deg) * scale_factor * minute_pointer_length),
                (self.center[1] - math.cos(minute_deg) * scale_factor * minute_pointer_length)))
                # hour_pointer_end =  self.rotate_point_to_position(self.center,hour_deg,hour_pointer_length)
                # print(hour_pointer_end)
                # pygame.draw.line(self.screen, (50, 255, 0), self.center,hour_pointer_end)
                # pygame.draw.line(self.screen, (255, 0, 0), self.center, (
                #     (self.center[0] + math.sin(second_deg) * scale_factor * second_pointer_length),
                #     (self.center[1] - math.cos(second_deg) * scale_factor * second_pointer_length)))
                # pygame.draw.line(self.screen, (255, 200, 0), self.center, (
                ...
                pygame.draw.rect(self.screen, (250, 160, 50),
                                 (woff, hoff, self.width - woff * 2, self.height - hoff * 2))
                pygame.draw.rect(self.screen, (69, 160, 50),
                                 (woff * 2, hoff * 2, self.width - woff * 4, self.height - hoff * 4))
                textsurf = self.myfont.render(str(self.time_manager), False, (250, 0, 0))

                centered_rect = textsurf.get_rect(center=(self.width // 2,
                                                          self.height // 2))  # https://stackoverflow.com/questions/23982907/python-library-pygame-centering-text#39580531
